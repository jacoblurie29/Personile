...
const adminLinks = [{ linkTitle: "Profile", linkRoute: "/profile" }];
...
export default function DashboardNavigation({ component }: Props) {
  ...
  const { seconds, minutes, pause, restart, resume, isRunning } = useTimer({
    ...
    >
      ...
      >
        ...
        <List>
          ...
          <ListItem key={"logout"} disablePadding sx={navButtonStyles}>
            <ListItemButton
              sx={{
                minHeight: 24,
                justifyContent: open ? "initial" : "center",
          ...
          {isRunning && (
            ...
            >
              <TimerIcon
                sx={{
                  color: currentTimerMax == 25 ? "success.dark" : "error.dark",
                }}
              />
              <Typography
                sx={{
      ...
      >
        ...
        {!loading && component === "sprint" && (
          <SprintView page={currentSprintPage} />
        )}
