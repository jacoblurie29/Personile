...
export default function ProfileView() {
  ...
  const tasksCompletedYesterday = allTasks?.filter((t) => {
    return (
      Date.now() - Date.parse(t.dateFinished) <
        86400000 + new Date().getTime() &&
      Date.now() - Date.parse(t.dateFinished) > new Date().getTime()
    );
  }).length;
  ...
  const handleEditUser = (data: FieldValues) => {
    ...
    setLoading(true);
    dispatch(updateUserAsync({ newData: userDto }))
    ...
    <>
      <Box display="flex" justifyContent="center" marginTop="100px">
        <Box
          sx={{
            ...
            width: "150px",
            borderRadius: "75px",
            backgroundColor: "rgba(153, 208, 211, 0.8)",
            display: "flex",
            justifyContent: "center",
            alignItems: "center",
            ":hover": {
              cursor: "pointer",
              backgroundColor: "rgba(153, 208, 211, 1.0)",
            },
          }}
        ...
        >
          {!hover && !isEdit && (
            <Typography variant="h1" fontSize={"50px"} sx={{ color: "white" }}>
          ...
          {hover && !isEdit && (
            <Typography variant="h1" fontSize={"50px"} sx={{ color: "white" }}>
              <EditIcon sx={{ fontSize: "60px", marginTop: "10px" }} />
            </Typography>
          )}
          {isEdit && (
            <Typography variant="h1" fontSize={"50px"} sx={{ color: "white" }}>
              <ClearIcon sx={{ fontSize: "60px", marginTop: "10px" }} />
            </Typography>
          )}
        </Box>
      </Box>
      <FormProvider {...methods}>
        ...
        >
          <Box
            ...
            width={"20%"}
            margin="auto"
            sx={{ paddingTop: isEdit ? "20px" : "0px" }}
          >
            {!isEdit && (
              <Typography variant="h1" sx={{ color: "grey.600" }}>
            ...
            {isEdit && (
              <>
                <EditProfileTextField
                  ...
                  label="First Name"
                  name="firstName"
                  editvalue={user?.firstName}
                />
                <EditProfileTextField
                  control={control}
                  label="Last Name"
                  name="lastName"
                  editvalue={user?.lastName}
                />
              </>
            )}
          </Box>
      ...
      <Box sx={{ textAlign: "center", marginTop: "50px" }}>
        <Typography sx={{ color: "primary.main" }} variant="h1">
          {"You have "}
          <Box component={"span"} sx={{ color: "#c99eec" }}>
            {remainingTasks}
          </Box>
          {" remaining tasks on "}
          <Box component={"span"} sx={{ color: "#c99eec" }}>
          ...
          {(user?.boards.length || 1) == 1 ? " board." : " boards."}
        </Typography>
        <Typography sx={{ color: "primary.main" }} variant="h1">
          ...
          {tasksCompletedYesterday == 1 ? " task." : " tasks."}
        </Typography>
        <Typography sx={{ color: "primary.main" }} variant="h1">
          {"In the last week you have completed "}
          <Box component={"span"} sx={{ color: "#c99eec" }}>
            {tasksCompletedThisWeek}
          </Box>
          {tasksCompletedThisWeek == 1 ? " task." : " tasks."}
        </Typography>
        <Button
          variant="contained"
          sx={{ marginTop: "25px" }}
