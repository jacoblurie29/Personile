...
import {
  convertDateAndTimeStringToSeconds,
  convertTimeStringToSeconds,
} from "app/util/dateUtil";
...
export default function RecentActivityCard({ boards }: Props) {
  ...
  const recentActivity =
    useAppSelector((state) => state.user.recentActivity) || [];
      ...
      {status.includes("pendingGetRecentActivity") ? (
        ...
        <Box sx={{ overflowY: "auto", maxHeight: "75%" }}>
          {[...recentActivity]
            ?.sort(
              (a, b) =>
                convertDateAndTimeStringToSeconds(a.date, a.time) -
                convertDateAndTimeStringToSeconds(b.date, b.time)
            )
            .reverse()
            .map((event, index) => (
              <RecentWidgetActivityCard
                event={event}
                index={index}
                key={index}
                max={recentActivity.length - 1}
              />
            ))}
        </Box>
      )}
