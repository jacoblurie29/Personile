...
export default function SprintVisualizer({
  overflowOption,
  sprintLength,
  totalLength,
}: Props) {
  ...
  const numberOfSprints =
    totalLength % sprintLength > Math.floor(sprintLength / 2)
      ? Math.ceil(totalLength / sprintLength)
      : Math.floor(totalLength / sprintLength);
  ...
  return (
    <>
      <Grid container display={"flex"} justifyContent={"center"}>
        {calculateSprintValues(numberOfSprints).map(
          (value, index) =>
            ((index == 0 ||
              index == 1 ||
              index == calculateSprintValues(numberOfSprints).length - 1 ||
              index == calculateSprintValues(numberOfSprints).length - 2) && (
              <Grid
                item
                sx={{
                  padding: "5px 10px",
            ...
            ((index == 0 ||
              ...
              <Grid
                ...
                sx={{
                  ...
                  textAlign: "center",
                  borderRadius: calculatedPredictionBorders(
                    index,
                    numberOfSprints - 1
                  ),
                }}
