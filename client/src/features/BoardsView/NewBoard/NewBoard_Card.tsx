...
export default function NewBoardCard(props: Props) {
  ...
  const [currentName, setCurrentName] = useState<string>(
    props.editBoard?.name || ""
  );
  const [currentDescription, setCurrentDecription] = useState<string>(
    props.editBoard?.description || ""
  );
  const [endDateEnabled, setEndDateEnabled] = useState<boolean>(
    (props.editBoard?.endDate !== "" && props.editBoard !== undefined) || false
  );
  const [newGoals, setNewGoals] = useState<string[]>(
    props.editBoard?.goals.map((g) => g.details) || []
  );
  const [newMilestones, setNewMilestones] = useState<string[]>(
    props.editBoard?.milestones.map((m) => m.description) || []
  );
  const [newMilestoneDates, setNewMilestoneDates] = useState<string[]>(
    props.editBoard?.milestones.map((m) => m.dueDate) || []
  );
  const [enableMilestoneDates, setEnableMilestoneDates] = useState<boolean[]>(
    props.editBoard?.milestones.map((m) => m.dueDate !== "") || []
  );
  const [newMilestoneHardDeadline, setNewMilestoneHardDeadline] = useState<
    boolean[]
  >(props.editBoard?.milestones.map((m) => m.hardDeadline) || []);
  const [boardEndDate, setBoardEndDate] = useState<string[]>(
    props.editBoard?.endDate !== "" && props.editBoard?.endDate !== undefined
      ? [props.editBoard?.endDate]
      : [""]
  );
  const [currentScreen, setCurrentScreen] = useState<number>(1);
  const [customLengthChecked, setCustomLengthChecked] =
    useState<boolean>(false);
  const [currentCustomSprintLength, setCurrentCustomSprintLength] = useState<
    number | undefined
  >(undefined);
  const [overflow, setOverflow] = useState<string>("start");
  const [sprintOptionSelected, setSprintOptionSelected] = useState<
    number | undefined
  >(undefined);
  const [firstScreenAnimation, setFirstScreenAnimation] =
    useState<boolean>(true);
  const [secondScreenAnimation, setSecondScreenAnimation] =
    useState<boolean>(true);
  ...
  async function submitForm(data: FieldValues) {
    ...
    newMilestones.forEach((milestone, index) => {
      if (milestone !== "") {
        milestoneArray.push({
          ...
          status: "Incomplete",
          dueDate: (enableMilestoneDates[index]
            ? newMilestoneDates[index].toString() === ""
    ...
    if (props.editBoard == undefined) {
      ...
      var sprintDaysLength = () => {
        if (customLengthChecked) {
          ...
          switch (sprintOptionSelected) {
            case 0:
              return calculateSprintShortLength(
                calculateTotalLength(boardEndDate[0])
              );
              break;
            case 1:
              return calculateSprintRecommendedLength(
                calculateTotalLength(boardEndDate[0])
              );
              break;
            case 2:
              return calculateSprintLongLength(
                calculateTotalLength(boardEndDate[0])
              );
              break;
            default:
      ...
      dispatch(addBoardAsync({ userId: userId, board: newBoard })).catch(
      ...
      dispatch(
        updateBoardAsync({
          userId: userId,
          boardId: props.editBoard.boardEntityId,
          updateBoardDto: editBoard,
  ...
  const calculateNumberOfSprints = (
    totalDays: number,
    sprintLength: number
  ) => {
            ...
            <Box
              ...
              onSubmit={
                props.editBoard == undefined
                  ? handleSubmit(handleNextScreen)
                  : handleSubmit(submitForm)
              }
              sx={{ mt: 1 }}
            >
              <Typography variant="h2" margin="20px 20px 10px 20px">
                {props.editBoard == undefined ? "New Board" : "Edit Board"}
              </Typography>
              <Divider />
                    ...
                    <Grid container alignItems="center">
                      ...
                      <Grid item xs={11} paddingTop="10px" paddingBottom="10px">
                        <NewBoardDatePicker
                          ...
                          name={"boardEndDate"}
                          value={boardEndDate[0]}
                          index={0}
                          onChange={handleChangeBoardEndDate}
                        />
                      </Grid>
                    </Grid>
                  )}
                    ...
                    <Grow in={true} key={"goalGrow-" + index}>
                      ...
                      >
                        <Grid
                          ...
                          paddingBottom="10px"
                        >
                          <NewBoardAttributeTextField
                            value={newGoals[index]}
                            id={"goal" + index}
                            label={"Goal #" + (index + 1)}
                            name={"goal" + index}
                            index={index}
                        ...
                        <Grid item xs={18}>
                          ...
                          >
                            ...
                            <Grid item xs={2} textAlign="left">
                              <Switch
                                sx={{ color: "primary.light" }}
                                checked={enableMilestoneDates[index]}
                                onChange={() =>
                                  handleChangeEnableMilestoneDate(index)
                                }
                              />
                            </Grid>
                            <Grid
                            ...
                            >
                              <NewBoardDatePicker
                                ...
                                id={"date" + index}
                                label={
                                  newMilestoneHardDeadline[index]
                                    ? "Deadline"
                                    : "Goal"
                                }
                                name={"date" + index}
                                value={newMilestoneDates[index]}
                                index={index}
                            ...
                            >
                              <Typography
                                ...
                                textAlign="center"
                                color={
                                  enableMilestoneDates[index]
                                    ? "grey.900"
                                    : "grey.400"
                                }
                              >
                                Is this is a hard deadline?
                              </Typography>
                            </Grid>
                            <Grid
                              item
                            ...
                            >
                              ...
                              >
                                <ToggleButton
                                  selected={
                                    enableMilestoneDates[index] &&
                                    newMilestoneHardDeadline[index] == true
                                  }
                                  onClick={() =>
                                    handleChangeHardDeadline(index, true)
                                  }
                                  value="Yes"
                                >
                                  Yes
                                ...
                                <ToggleButton
                                  selected={
                                    enableMilestoneDates[index] &&
                                    newMilestoneHardDeadline[index] == false
                                  }
                                  onClick={() =>
                                    handleChangeHardDeadline(index, false)
                                  }
                                  value="No"
                                >
                                  No
                          ...
                          <Box flexGrow={1} textAlign="center">
                            <IconButton
                              ...
                              onClick={() => {
                                ...
                                handleDeleteMilestoneDate(index);
                              }}
                            >
                              <ClearIcon sx={{ fontSize: "20px" }} />
                            </IconButton>
                ...
                {props.editBoard == undefined ? (
                  <>
                    <LoadingButton
                      ...
                      onClick={() => props.setNewBoardState(false)}
                    >
                  ...
                  <>
                    <LoadingButton
                      ...
                      sx={{
                        background:
                          "linear-gradient(232deg, rgba(173,173,173,1) 0%, rgba(158,158,158,1) 100%)",
                        borderRadius: "5px",
                        mr: "10px",
                      }}
                      onClick={() => props.setNewBoardState(false)}
                    ...
                    <LoadingButton
                      ...
                      onClick={() =>
                        handleDeleteBoard(props.editBoard?.boardEntityId || "")
                      }
                    >
                      <DeleteIcon sx={{ color: "background.paper" }} />
                    </LoadingButton>
          ...
          <Card sx={{ width: "60%", maxWidth: "800px" }}>
            ...
            >
              <Grid container>
                <Grid item xs={6}>
                  <Typography variant="h2" margin="20px 20px 10px 20px">
                    Select Sprint Length
                  </Typography>
                </Grid>
                <Grid item xs={6} sx={{ textAlign: "right" }}>
                  <Typography variant="h5" margin="20px 20px 10px 20px">
                    {calculateTotalLength(boardEndDate[0]) !== 0 &&
                      formatDateString(new Date().toString()) +
                        " to " +
                        formatDateString(boardEndDate[0])}
                    <b>
                      &nbsp;&nbsp;
                      {calculateTotalLength(boardEndDate[0]) !== 0
                        ? "(" + calculateTotalLength(boardEndDate[0]) + " days)"
                        : "No end date"}
                    </b>
                  </Typography>
                </Grid>
              ...
              >
                <Grid
                  item
                  xs={7}
                  sx={{ textAlign: "left", margin: "20px 6px 6px 6px" }}
                >
                  <Card sx={{ textAlign: "center" }} elevation={3}>
                    <Grid container>
                      ...
                      <Grid item xs={2} sx={{ paddingTop: "2px" }}>
                        {sprintOptionSelected == 0 && !customLengthChecked && (
                    ...
                    <Box paddingLeft="5px">
                      ...
                      >
                        {calculateSprintShortLength(
                          calculateTotalLength(boardEndDate[0])
                        )}
                      </Typography>
                      <Typography
                        variant="h3"
                        sx={{ margin: "4px", display: "inline-block" }}
                      >
                        Days
                      </Typography>
                    </Box>
                    <Typography variant="h5" sx={{ margin: "0px 4px 4px 4px" }}>
                      {calculateTotalLength(boardEndDate[0]) /
                        calculateSprintShortLength(
                          calculateTotalLength(boardEndDate[0])
                        ) !==
                    ...
                    <Typography variant="subtitle2" padding="5px">
                      Divide your board into smaller sections. Great for boards
                      with smaller tasks.
                    </Typography>
                    <Button
                      onClick={() => setSprintOptionSelected(0)}
                      fullWidth
                      sx={{
                        backgroundColor: customLengthChecked
                          ? "grey.400"
                          : "primary.main",
                        borderRadius: "0px 0px 5px 5px",
                        ":hover": { backgroundColor: "primary.dark" },
                      }}
                      disabled={customLengthChecked}
                    >
                      <Typography
                        variant="h6"
                ...
                <Grid
                  item
                  xs={9}
                  sx={{ textAlign: "left", margin: "10px 6px 6px 6px" }}
                >
                  <Card sx={{ textAlign: "center" }} elevation={3}>
                    <Grid container>
                      <Grid item xs={2}></Grid>
                      ...
                      <Grid item xs={2} sx={{ paddingTop: "2px" }}>
                        {sprintOptionSelected == 1 && !customLengthChecked && (
                          <CheckCircleIcon sx={{ color: "success.main" }} />
                        )}
                      </Grid>
                    </Grid>
                    ...
                    <Box paddingLeft="5px">
                      ...
                      >
                        {calculateSprintRecommendedLength(
                          calculateTotalLength(boardEndDate[0])
                        )}
                      </Typography>
                      <Typography
                    ...
                    <Divider />
                    <Typography variant="subtitle2" padding="5px">
                      Not sure where to start? We've got you covered. Our
                      algorithm reccomends you start here!
                    </Typography>
                    <Button
                      onClick={() => setSprintOptionSelected(1)}
                      fullWidth
                      sx={{
                        backgroundColor: customLengthChecked
                          ? "grey.400"
                ...
                <Grid
                  item
                  xs={7}
                  sx={{ textAlign: "left", margin: "20px 6px 0px 6px" }}
                >
                  <Card sx={{ textAlign: "center" }} elevation={3}>
                    <Grid container>
                      <Grid item xs={2}></Grid>
                      ...
                      <Grid item xs={2} sx={{ paddingTop: "2px" }}>
                        {sprintOptionSelected == 2 && !customLengthChecked && (
                          <CheckCircleIcon sx={{ color: "success.main" }} />
                        )}
                      </Grid>
                    </Grid>
                    ...
                    <Divider />
                    <Typography variant="subtitle2" padding="5px">
                      Divide your board into longer sections. Better for boards
                      with broader tasks
                    </Typography>
                    <Button
                      onClick={() => setSprintOptionSelected(2)}
                      fullWidth
                      sx={{
                        backgroundColor: customLengthChecked
                          ? "grey.400"
                    ...
                    <Box>
                      <Grid container width="fit-content">
                        <Grid item xs={1} marginTop="auto" marginBottom="auto">
                          <Checkbox
                            sx={{ color: "grey.600", marginRight: "10px" }}
                            value={customLengthChecked}
                            onChange={(event, checked) => {
                              setCustomLengthChecked(checked);
                              setCurrentCustomSprintLength(
                                calculateSprintRecommendedLength(
                                  calculateTotalLength(boardEndDate[0])
                                )
                              );
                        ...
                        <Grid item xs={6} marginTop="auto" marginBottom="auto">
                          <TextField
                            value={
                              customLengthChecked
                                ? currentCustomSprintLength
                                : ""
                            }
                            onChange={(event) => {
                              event.target.value == ""
                                ? setCurrentCustomSprintLength(undefined)
                            ...
                            type="number"
                            helperText={
                              currentCustomSprintLength !== undefined &&
                              endDateEnabled &&
                              currentCustomSprintLength > 0 &&
                              customLengthChecked
                                ? calculateNumberOfSprints(
                                    calculateTotalLength(boardEndDate[0]),
                                    currentCustomSprintLength
              ...
              <Box padding="20px">
                ...
                <SprintVisualizer
                  ...
                  sprintLength={findSprintLength() || 0}
                  totalLength={calculateTotalLength(boardEndDate[0]) || 0}
                />
              </Box>
              <Box>
                <Grid container>
