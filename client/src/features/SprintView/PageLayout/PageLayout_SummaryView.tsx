          ...
          <Box flexGrow={1}>
            <Grid container>
              <Grid item xs={8}>
                <Typography
                  variant="h2"
                  sx={{ fontWeight: "700", color: "white", padding: "15px" }}
                >
                  {"All Tasks"}
                </Typography>
              </Grid>
                    ...
                    <Fragment key={"Fragment" + task.taskEntityId + taskIndex}>
                      {!taskToBeEditedId.includes(task.taskEntityId) ? (
                        <SummaryCard
                          ...
                          sprintId={sprint.sprintEntityId}
                          animationIndex={
                            taskIndex + sprint.tasks.length * sprintIndex
                          }
                          max={sprint.tasks.length - 1}
                          key={"SummaryCard-" + taskIndex}
                          toggleEditTask={toggleEditTask}
                          handleMoveTask={changeOrderOfTask}
                        />
                      ) : (
                        <SummaryNewEditTaskCard
                          toggleEditTask={toggleEditTask}
                          key={"Editor" + task.taskEntityId + taskIndex}
                          setNewTask={setNewTask}
                          editTask={task}
                        />
                      )}
