...
export default function PageLayoutTaskColumnView({
  tasks,
  stateTitle,
  sprintId,
  toggleEditTask,
    ...
    <>
      ...
      >
        {stateTitle == "New" && (
          <Typography
            variant="h2"
            sx={{ fontWeight: "700", color: "grey.50", padding: "15px" }}
          >
            {stateTitle}&nbsp;&#128221;
          </Typography>
        )}
        ...
        <Fragment key={"Fragment" + task.taskEntityId + index + sprintId}>
          {!tasksToBeEdited.includes(task.taskEntityId) ? (
            <ViewTaskCard
              task={task}
              sprintId={sprintId}
              key={"Task" + task.taskEntityId + index + sprintId}
              toggleEditTask={toggleEditTask}
              indexForAnimation={index}
              index={index}
              max={tasks.length - 1}
              previousIndex={calculatePreviousTaskOrderNumber(
                task.taskEntityId
              )}
              nextIndex={calculateNextTaskOrderNumber(task.taskEntityId)}
            />
          ) : (
            <NewEditTaskCard
              toggleEditTask={toggleEditTask}
              key={"Editor" + task.taskEntityId + index + sprintId}
              setNewTask={setNewTask}
              editTask={task}
            />
          )}

          {index === tasks.length - 1 && stateTitle === "New" && !newTask && (
            <PageLayoutNewTaskButton
              key={"NewTaskButton" + task.taskEntityId + index + sprintId}
              addNewTaskOnClick={handleNewTask}
              index={tasks.length}
            />
          )}

          {index === tasks.length - 1 && newTask && (
            <NewEditTaskCard
              key={"NewTask" + task.taskEntityId + index + sprintId}
              setNewTask={setNewTask}
              toggleEditTask={toggleEditTask}
            />
          )}
      ...
      {tasks.length === 0 && stateTitle === "New" && !newTask && (
        <PageLayoutNewTaskButton
          addNewTaskOnClick={handleNewTask}
          index={tasks.length}
        />
