...
export default function NewEditTaskMilestoneSelect(props: formProps) {
  ...
  const { fieldState, field } = useController({
    ...props,
    defaultValue: props.editvalue?.split("|") || [],
  });
  ...
  return (
    <FormControl variant="outlined" fullWidth sx={selectStyles}>
      <InputLabel id="demo-multiple-checkbox-label" key="label">
        Milestones
      </InputLabel>
      <Select
        ...
        defaultValue={
          props.editvalue !== undefined && props.editvalue !== ""
            ? [...props.editvalue.split("|")]
            : []
        }
        error={!!fieldState.error?.message}
        value={field.value[0] !== "" ? field.value : []}
        input={<OutlinedInput label="Milestones" key="box" />}
        MenuProps={MenuProps}
            ...
            {field.value[0] !== "" &&
              field.value.map((value: string, index: number) => (
                <Chip
                  ...
                  variant="filled"
                  label={
                    props.board.milestones.find(
                      (m) => m.milestoneEntityId == value
                    )?.description
                  }
                />
              ))}
