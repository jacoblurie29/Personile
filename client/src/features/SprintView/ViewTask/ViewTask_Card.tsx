...
import {
  changeTaskStateAsync,
  moveTaskOrderAsync,
  removeTaskFromSprintAsync,
  updateTaskAsync,
...
export default function ViewTaskCard({
  ...
  const {
    ...
    isExpanded: expanded,
    currentBoard,
  } = useAppSelector((state) => state.sprintView);
  const userEntityId = useAppSelector(
    (state) => state.user.userData?.userEntityId
  );
    ...
    if (currentState != newState) {
      ...
      dispatch(
        changeTaskStateAsync({
          ...
          newOrder: currentTask.order!.toString(),
          oldOrder: currentTask.order!.toString(),
        })
      );
  ...
  const changeOrderOfTask = (
    taskId: string,
    sprintId: string,
    oldOrderLocation: number,
    newOrderLocation: number
  ) => {
    // null checks
    ...
    dispatch(
      moveTaskOrderAsync({
        ...
        sprintId: sprintId,
        taskId: taskId,
        oldOrder: oldOrderLocation.toString(),
        newOrder: newOrderLocation.toString(),
      })
    ...
    <Zoom in={zoom} timeout={zoom ? (indexForAnimation + 1) * 500 : 500}>
      <Accordion
        sx={{
          ...
          borderRight: "1px",
          borderBottom: "1px",
          borderTop: expanded?.includes(task.taskEntityId) ? "1px" : "0px",
          borderStyle: "solid",
          borderColor: "grey.200",
        }}
        elevation={0}
        expanded={expanded?.includes(task.taskEntityId)}
        onChange={handleChange(task.taskEntityId)}
        key={task.taskEntityId}
      >
        <AccordionSummary
          sx={{
            ".MuiAccordionSummary-content": { margin: "10px 0px !important" },
          }}
        >
          <Box flexGrow={1}>
            <Grid container columns={6}>
              <Grid item xs={6}>
                <ViewTaskStateDisplay
                  title={task.name}
              ...
              <Grid item xs={6}>
                <Grid container>
                  <Grid item xs={9}>
                    <Typography
                      sx={
                        expanded?.includes(task.taskEntityId)
                          ? descriptionStylesExpanded
                          : descriptionStylesNotExpanded
                      }
                      color="grey.500"
                    >
                      {task.description}
                  ...
                  >
                    <IconButton
                      disabled={index == 0 || status.includes("pending")}
                    ...
                    <IconButton
                      disabled={index == max || status.includes("pending")}
                      sx={{ height: "fit-content", margin: "auto" }}
                      onClick={(event) => {
                        onClickDelete(event);
                        changeOrderOfTask(
        ...
        <AccordionDetails sx={{ alignItems: "center" }}>
          ...
          <Grid container sx={{ display: "flex", width: "auto" }}>
            <Grid item xs={6}>
              ...
              >
                <ViewTaskStateToggleButton
                  sprintId={sprintId}
                  startingState={task.currentState}
                  task={task}
            ...
            <Grid item xs={6}>
              ...
              >
                <ViewTaskActionButton
                  task={task}
                  toggleEditTask={toggleEditTask}
                  setZoom={setZoom}
                  setMoveSprint={setMoveSprint}
                />
                {moveSprint && (
                  <TaskChangeSprintCard
                    setMoveSprint={setMoveSprint}
                    task={task}
