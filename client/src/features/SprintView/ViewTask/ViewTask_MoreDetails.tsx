import { Grid, Typography, Chip, LinearProgress, linearProgressClasses, styled, Tooltip, Box, Collapse, IconButton } from "@mui/material";
...
export default function ViewTaskMoreDetails({ focusedTask }: Props) {
    ...
    return (
        <Box paddingBottom='10px'>
            ...
            <Box width={"100%"} display={'flex'} justifyContent={'center'} alignItems={'end'} sx={{":hover": {backgroundColor: 'grey.50', cursor: 'pointer'}, borderRadius: '5px'}}
                onClick={() => {setExpanded(!expanded)}}>
                {expanded ? <ArrowDropUpIcon sx={{color: 'grey.400'}} /> : <ArrowDropDownIcon sx={{color: 'grey.400'}}/>}
                <Typography sx={{padding: '0px'}} variant="caption">Tags, Links, Effort, Milestones</Typography>
                {expanded ? <ArrowDropUpIcon sx={{color: 'grey.400'}} /> : <ArrowDropDownIcon sx={{color: 'grey.400'}}/>}
            </Box>
