...
export default function TaskChangeSprintCard({
  setMoveSprint,
  task,
  oldSprintId,
}: Props) {
  ...
  const [currentSprintButton, setCurrentSprintButton] =
    useState<string>(oldSprintId);

  const changeTaskSprint = (newSprintId: string) => {
    setMoveSprint(false);
    dispatch(
      changeTaskSprintAsync({
        userId: userEntityId || "",
          ...
          <>
                    ...
                    <Grid container width={"100%"} columns={18}>
                      <Grid item xs={6} padding={"0px 20px"}>
                        <Typography
                          ...
                          flexGrow={1}
                        >
                          {formatDateStringNoYear(sprint.startDate) +
                            " - " +
                            formatDateStringNoYear(sprint.endDate)}
                        </Typography>
                      </Grid>
                      <Grid item xs={6} padding={"0px 20px"}>
                        <Grid
                        ...
                        <Grid container>
                          <Grid item xs={10}>
                            <Button
                              ...
                              onClick={() =>
                                setCurrentSprintButton(sprint.sprintEntityId)
                              }
                            >
                              Select
                            </Button>
                          ...
                          <Grid
                            item
                            xs={2}
                            sx={{ textAlign: "left", paddingTop: "5px" }}
                          >
                            {currentSprintButton == sprint.sprintEntityId &&
                              currentSprintButton == oldSprintId && (
                                <CheckCircleIcon sx={{ color: "grey.400" }} />
                              )}
                            {currentSprintButton == sprint.sprintEntityId &&
                              currentSprintButton != oldSprintId && (
                                <CheckCircleIcon
                                  sx={{ color: "rgba(58,203,152,10)" }}
                                />
                              )}
                          </Grid>
                        </Grid>
            ...
            >
              <LoadingButton
                ...
                onClick={() => setMoveSprint(false)}
              >
                <ArrowBackIcon sx={{ color: "background.paper" }} />
              </LoadingButton>
              <LoadingButton
                disabled={currentSprintButton == oldSprintId}
                type="submit"
                key={"next"}
                variant="contained"
                onClick={() => changeTaskSprint(currentSprintButton)}
                sx={{
                  borderRadius: "5px",
                  background:
                    currentSprintButton == oldSprintId
